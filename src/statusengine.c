...
static gboolean parent_hosts_foreach_callback(gpointer _name, gpointer _hostsmember, gpointer _parent_hosts_array) {
    ...
    json_object_array_add(parent_hosts_array,
                          (hostsmember->name != NULL ? json_object_new_string(hostsmember->name) : NULL));
    return FALSE;
}
...
static gboolean hostgroup_foreach_callback(gpointer key, gpointer _hostgroupmember, gpointer _hostgroup_members_array) {
    ...
    json_object_array_add(hostgroup_members_array,
                          (hostgroupmember->name != NULL ? json_object_new_string(hostgroupmember->name) : NULL));
    return FALSE;
}
...
int statusengine_send_job(char *queue, char *data) {
    ...
    for (i = 0; i < gearman_server_num; i++) {
        gearman_return_t ret = gearman_client_do_background(gearman_server_list[i]->client, queue, NULL, (void *) data,
                                                            (size_t) strlen(data), NULL);
        ...
        if (ret != GEARMAN_SUCCESS) {
            ...
            if (gearman_server_list[i]->errors == 0 && do_logging) {
                ...
                logswitch(NSLOG_INFO_MESSAGE, "sending to gearmand %s failed: %s\n", gearman_server_list[i]->server,
                          (char *) gearman_client_error(gearman_server_list[i]->client));
            }
...
int nebmodule_init(int flags, char *args, nebmodule *handle) {
    ...
    neb_set_module_info(statusengine_module_handle, NEBMODULE_MODINFO_TITLE,
                        "Copyright (c) 2014 - present Daniel Ziegler");
    ...
    neb_register_callback(NEBCALLBACK_CONTACT_NOTIFICATION_DATA, statusengine_module_handle, 0,
                          statusengine_handle_data);
    neb_register_callback(NEBCALLBACK_CONTACT_NOTIFICATION_METHOD_DATA, statusengine_module_handle, 0,
                          statusengine_handle_data);
...
int statusengine_process_config_var(char *arg) {
    ...
    if (!strcmp(var, "use_host_status_data")) {
        ...
        debug = atoi(val);
        logswitch(NSLOG_INFO_MESSAGE, "start with debug mode enabled");
...
void flush_servicestatus_cache() {
    ...
    if (debug) {
        char buffer[1024];
        sprintf(
                buffer, "Flush %d records as bunch to queue %s",
                json_object_array_length(servicestatus_bulk_cache),
                "statusngin_servicestatus"
        );
        logswitch(NSLOG_INFO_MESSAGE, buffer);
    }
...
void flush_hoststatus_cache() {
    ...
    if (debug) {
        ...
        sprintf(
                buffer, "Flush %d records as bunch to queue %s",
                json_object_array_length(hoststatus_bulk_cache),
...
void flush_servicechecks_cache() {
    ...
    if (use_service_check_data) {
        if (debug) {
            ...
            sprintf(
                    buffer, "Flush %d records as bunch to queue %s",
                    json_object_array_length(servicechecks_bulk_cache),
...
void flush_serviceperfdata_cache() {
    ...
    if (debug) {
        ...
        sprintf(
                buffer, "Flush %d records as bunch to queue %s",
                json_object_array_length(serviceperfdata_bulk_cache),
...
void flush_hostchecks_cache() {
    ...
    if (debug) {
        ...
        sprintf(
                buffer, "Flush %d records as bunch to queue %s",
                json_object_array_length(hostchecks_bulk_cache),
...
void flush_statechanges_cache() {
    ...
    if (debug) {
        ...
        sprintf(
                buffer, "Flush %d records as bunch to queue %s",
                json_object_array_length(statechanges_bulk_cache),
...
void flush_logentries_cache() {
    ...
    if (debug) {
        ...
        sprintf(
                buffer, "Flush %d records as bunch to queue %s",
                json_object_array_length(logentries_bulk_cache),
...
void flush_externalcommands_cache() {
    ...
    if (debug) {
        ...
        sprintf(
                buffer, "Flush %d records as bunch to queue %s",
                json_object_array_length(externalcommands_bulk_cache),
...
void flush_acknowledgements_cache() {
    ...
    if (debug) {
        ...
        sprintf(
                buffer, "Flush %d records as bunch to queue %s",
                json_object_array_length(acknowledgements_bulk_cache),
...
void flush_downtimes_cache() {
    ...
    if(debug) {
        ...
        sprintf(
                buffer, "Flush %d records as bunch to queue %s",
                json_object_array_length(downtimes_bulk_cache),
...
void flush_objects_cache() {
    ...
    if (debug) {
        ...
        sprintf(
                buffer, "Flush %d records as bunch to queue %s",
                json_object_array_length(objects_bulk_cache),
                "statusngin_objects"
        );
        logswitch(NSLOG_INFO_MESSAGE, buffer);
    }
...
void check_all_caches() {
    if (json_object_array_length(servicestatus_bulk_cache) > BULK_SIZE ||
        last_servicestatus_bulk_flush < ((int) time(NULL) - BULK_TIMEOUT)) {
        if (json_object_array_length(servicestatus_bulk_cache) > 0) {
            flush_servicestatus_cache();
        }
    }

    if (json_object_array_length(hoststatus_bulk_cache) > BULK_SIZE ||
        last_hoststatus_bulk_flush < ((int) time(NULL) - BULK_TIMEOUT)) {
        if (json_object_array_length(hoststatus_bulk_cache) > 0) {
            flush_hoststatus_cache();
        }
    }

    if (json_object_array_length(servicechecks_bulk_cache) > BULK_SIZE ||
        last_servicechecks_bulk_flush < ((int) time(NULL) - BULK_TIMEOUT)) {
        if (json_object_array_length(servicechecks_bulk_cache) > 0) {
            flush_servicechecks_cache();
        }
    }

    if (json_object_array_length(serviceperfdata_bulk_cache) > BULK_SIZE ||
        last_serviceperfdata_bulk_flush < ((int) time(NULL) - BULK_TIMEOUT)) {
        if (json_object_array_length(serviceperfdata_bulk_cache) > 0) {
            flush_serviceperfdata_cache();
        }
    }

    if (json_object_array_length(hostchecks_bulk_cache) > BULK_SIZE ||
        last_hostchecks_bulk_flush < ((int) time(NULL) - BULK_TIMEOUT)) {
        if (json_object_array_length(hostchecks_bulk_cache) > 0) {
            flush_hostchecks_cache();
        }
    }

    if (json_object_array_length(statechanges_bulk_cache) > BULK_SIZE ||
        last_statechanges_bulk_flush < ((int) time(NULL) - BULK_TIMEOUT)) {
        if (json_object_array_length(statechanges_bulk_cache) > 0) {
            flush_statechanges_cache();
        }
    }

    if (json_object_array_length(logentries_bulk_cache) > BULK_SIZE ||
        last_logentries_bulk_flush < ((int) time(NULL) - BULK_TIMEOUT)) {
        if (json_object_array_length(logentries_bulk_cache) > 0) {
            flush_logentries_cache();
        }
    }

    if (json_object_array_length(externalcommands_bulk_cache) > BULK_SIZE ||
        last_externalcommands_bulk_flush < ((int) time(NULL) - BULK_TIMEOUT)) {
        if (json_object_array_length(externalcommands_bulk_cache) > 0) {
            flush_externalcommands_cache();
        }
    }

    if (json_object_array_length(acknowledgements_bulk_cache) > BULK_SIZE ||
        last_acknowledgements_bulk_flush < ((int) time(NULL) - BULK_TIMEOUT)) {
    ...
    /*
    if (json_object_array_length(downtimes_bulk_cache) > BULK_SIZE ||
        last_downtimes_bulk_flush < ((int) time(NULL) - BULK_TIMEOUT)) {
...
*/
    ...
    if (json_object_array_length(objects_bulk_cache) > BULK_SIZE ||
        last_objets_bulk_flush < ((int) time(NULL) - BULK_TIMEOUT)) {
        if (json_object_array_length(objects_bulk_cache) > 0) {
            flush_objects_cache();
        }
    }
    ...
    switch (event_type) {
...
#endif
                json_object_object_add(processdata_object, "programmversion",
                                       json_object_new_string(get_program_version()));
        ...
        case NEBCALLBACK_HOST_STATUS_DATA:
            ...
            if ((hoststatusdata = (nebstruct_host_status_data *) data)) {
                ...
                json_object_array_add(hoststatus_bulk_cache, my_object);
                if (json_object_array_length(hoststatus_bulk_cache) > BULK_SIZE ||
                    last_hoststatus_bulk_flush < ((int) time(NULL) - BULK_TIMEOUT)) {
                    flush_hoststatus_cache();
                }
        ...
        case NEBCALLBACK_SERVICE_STATUS_DATA:
            ...
            if ((servicestatusdata = (nebstruct_service_status_data *) data)) {
                ...
                json_object_object_add(my_object, "timestamp",
                                       json_object_new_int(servicestatusdata->timestamp.tv_sec));
        ...
        case NEBCALLBACK_SERVICE_CHECK_DATA:
            ...
            if ((servicecheck = (nebstruct_service_check_data *) data)) {
                ...
                get_raw_command_line_r(get_global_macros(), nag_service->check_command_ptr, nag_service->check_command,
                                       &raw_command, 0);
#endif
                json_object_object_add(servicecheck_object, "command_line",
                                       (raw_command != NULL ? json_object_new_string(raw_command) : NULL));
                json_object_object_add(servicecheck_object, "command_name",
                                       (nag_service->check_command != NULL ? json_object_new_string(
                                               nag_service->check_command) : NULL));
                ...
                json_object_object_add(servicecheck_object, "start_time",
                                       json_object_new_int64(nag_servicecheck->start_time.tv_sec));
                json_object_object_add(servicecheck_object, "end_time",
                                       json_object_new_int64(nag_servicecheck->end_time.tv_sec));
        ...
        case NEBCALLBACK_HOST_CHECK_DATA:
            ...
            if ((hostcheck = (nebstruct_host_check_data *) data)) {
...
#endif
#if defined NAEMON105 || defined NAEMONMASTER
                get_raw_command_line_r(get_global_macros(), nag_host->check_command_ptr, nag_host->check_command,
                                       &raw_command, 0);
#endif
                json_object_object_add(hostcheck_object, "command_line",
                                       (raw_command != NULL ? json_object_new_string(raw_command) : NULL));
                json_object_object_add(hostcheck_object, "command_name",
                                       (nag_host->check_command != NULL ? json_object_new_string(
                                               nag_host->check_command) : NULL));
                ...
                json_object_object_add(hostcheck_object, "start_time",
                                       json_object_new_int64(nag_hostcheck->start_time.tv_sec));
                json_object_object_add(hostcheck_object, "end_time",
                                       json_object_new_int64(nag_hostcheck->end_time.tv_sec));
        ...
        case NEBCALLBACK_STATE_CHANGE_DATA:
            ...
            if ((statechange = (nebstruct_statechange_data *) data)) {
                ...
                json_object_object_add(statechange_object, "long_output",
                                       (statechange->output != NULL ? json_object_new_string(statechange->output)
                                                                    : NULL));
        ...
        case NEBCALLBACK_LOG_DATA:
            ...
            if ((logentry = (nebstruct_log_data *) data)) {
                ...
                json_object_object_add(logentry_object, "data",
                                       (logentry->data != NULL ? json_object_new_string(logentry->data) : NULL));
        ...
        case NEBCALLBACK_SYSTEM_COMMAND_DATA:
            ...
            if ((systemcommand = (nebstruct_system_command_data *) data)) {
                ...
                json_object_object_add(systemcommand_object, "command_line",
                                       (systemcommand->command_line != NULL ? json_object_new_string(
                                               systemcommand->command_line) : NULL));
                json_object_object_add(systemcommand_object, "output",
                                       (systemcommand->output != NULL ? json_object_new_string(systemcommand->output)
                                                                      : NULL));
                //I guess this is long output one day...
                json_object_object_add(systemcommand_object, "long_output",
                                       (systemcommand->output != NULL ? json_object_new_string(systemcommand->output)
                                                                      : NULL));

                json_object_object_add(systemcommand_object, "start_time",
                                       json_object_new_int64(systemcommand->start_time.tv_sec));
                json_object_object_add(systemcommand_object, "end_time",
                                       json_object_new_int64(systemcommand->end_time.tv_sec));
                json_object_object_add(systemcommand_object, "timeout", json_object_new_int64(systemcommand->timeout));
                json_object_object_add(systemcommand_object, "early_timeout",
                                       json_object_new_int64(systemcommand->early_timeout));
                json_object_object_add(systemcommand_object, "return_code",
                                       json_object_new_int64(systemcommand->return_code));

                json_object_object_add(systemcommand_object, "execution_time",
                                       json_object_new_double(systemcommand->execution_time));
        ...
        case NEBCALLBACK_COMMENT_DATA:
            ...
            if ((_comment = (nebstruct_comment_data *) data)) {
                ...
                json_object_object_add(comment_object, "host_name",
                                       (_comment->host_name != NULL ? json_object_new_string(_comment->host_name)
                                                                    : NULL));
                json_object_object_add(comment_object, "service_description",
                                       (_comment->service_description != NULL ? json_object_new_string(
                                               _comment->service_description) : NULL));
                json_object_object_add(comment_object, "author_name",
                                       (_comment->author_name != NULL ? json_object_new_string(_comment->author_name)
                                                                      : NULL));
                json_object_object_add(comment_object, "comment_data",
                                       (_comment->comment_data != NULL ? json_object_new_string(_comment->comment_data)
                                                                       : NULL));
        ...
        case NEBCALLBACK_EXTERNAL_COMMAND_DATA:
            ...
            if ((extcommand = (nebstruct_external_command_data *) data)) {
                ...
                json_object_object_add(extcommand_object, "command_string",
                                       (extcommand->command_string != NULL ? json_object_new_string(
                                               extcommand->command_string) : NULL));
                json_object_object_add(extcommand_object, "command_args",
                                       (extcommand->command_args != NULL ? json_object_new_string(
                                               extcommand->command_args) : NULL));
                json_object_object_add(extcommand_object, "command_type",
                                       json_object_new_int64(extcommand->command_type));
        ...
        case NEBCALLBACK_ACKNOWLEDGEMENT_DATA:
            ...
            if ((acknowledgement = (nebstruct_acknowledgement_data *) data)) {
                ...
                json_object_object_add(acknowledgement_object, "host_name",
                                       (acknowledgement->host_name != NULL ? json_object_new_string(
                                               acknowledgement->host_name) : NULL));
                json_object_object_add(acknowledgement_object, "service_description",
                                       (acknowledgement->service_description != NULL ? json_object_new_string(
                                               acknowledgement->service_description) : NULL));
                json_object_object_add(acknowledgement_object, "author_name",
                                       (acknowledgement->author_name != NULL ? json_object_new_string(
                                               acknowledgement->author_name) : NULL));
                json_object_object_add(acknowledgement_object, "comment_data",
                                       (acknowledgement->comment_data != NULL ? json_object_new_string(
                                               acknowledgement->comment_data) : NULL));

                json_object_object_add(acknowledgement_object, "acknowledgement_type",
                                       json_object_new_int64(acknowledgement->acknowledgement_type));
                json_object_object_add(acknowledgement_object, "state", json_object_new_int64(acknowledgement->state));
                json_object_object_add(acknowledgement_object, "is_sticky",
                                       json_object_new_int64(acknowledgement->is_sticky));
                json_object_object_add(acknowledgement_object, "persistent_comment",
                                       json_object_new_int64(acknowledgement->persistent_comment));
                json_object_object_add(acknowledgement_object, "notify_contacts",
                                       json_object_new_int64(acknowledgement->notify_contacts));
        ...
        case NEBCALLBACK_FLAPPING_DATA:
            ...
            if ((_flapping = (nebstruct_flapping_data *) data)) {
                ...
                json_object_object_add(flapping_object, "host_name",
                                       (_flapping->host_name != NULL ? json_object_new_string(_flapping->host_name)
                                                                     : NULL));
                json_object_object_add(flapping_object, "service_description",
                                       (_flapping->service_description != NULL ? json_object_new_string(
                                               _flapping->service_description) : NULL));
                ...
                json_object_object_add(flapping_object, "flapping_type",
                                       json_object_new_int64(_flapping->flapping_type));
                ...
                if (tmp_comment != NULL) {
                    json_object_object_add(flapping_object, "comment_entry_time",
                                           json_object_new_int64(tmp_comment->entry_time));
                } else {
                    json_object_object_add(flapping_object, "comment_entry_time", json_object_new_int64(0));
                }

                json_object_object_add(flapping_object, "percent_change",
                                       json_object_new_double(_flapping->percent_change));
                json_object_object_add(flapping_object, "high_threshold",
                                       json_object_new_double(_flapping->high_threshold));
                json_object_object_add(flapping_object, "low_threshold",
                                       json_object_new_double(_flapping->low_threshold));
        ...
        case NEBCALLBACK_DOWNTIME_DATA:
            ...
            if ((_downtime = (nebstruct_downtime_data *) data)) {
                ...
                json_object_object_add(downtime_object, "host_name",
                                       (_downtime->host_name != NULL ? json_object_new_string(_downtime->host_name)
                                                                     : NULL));
                json_object_object_add(downtime_object, "service_description",
                                       (_downtime->service_description != NULL ? json_object_new_string(
                                               _downtime->service_description) : NULL));
                json_object_object_add(downtime_object, "author_name",
                                       (_downtime->author_name != NULL ? json_object_new_string(_downtime->author_name)
                                                                       : NULL));
                json_object_object_add(downtime_object, "comment_data",
                                       (_downtime->comment_data != NULL ? json_object_new_string(
                                               _downtime->comment_data) : NULL));
                json_object_object_add(downtime_object, "host_name",
                                       (_downtime->host_name != NULL ? json_object_new_string(_downtime->host_name)
                                                                     : NULL));


                json_object_object_add(downtime_object, "downtime_type",
                                       json_object_new_int64(_downtime->downtime_type));
        ...
        case NEBCALLBACK_NOTIFICATION_DATA:
            ...
            if ((notificationdata = (nebstruct_notification_data *) data)) {
                ...
                json_object_object_add(notification_data_object, "host_name",
                                       (notificationdata->host_name != NULL ? json_object_new_string(
                                               notificationdata->host_name) : NULL));
                json_object_object_add(notification_data_object, "service_description",
                                       (notificationdata->service_description != NULL ? json_object_new_string(
                                               notificationdata->service_description) : NULL));
                json_object_object_add(notification_data_object, "output",
                                       (notificationdata->output != NULL ? json_object_new_string(
                                               notificationdata->output) : NULL));
                //May be some day, how knows?
                json_object_object_add(notification_data_object, "long_output",
                                       (notificationdata->output != NULL ? json_object_new_string(
                                               notificationdata->output) : NULL));
                json_object_object_add(notification_data_object, "ack_author",
                                       (notificationdata->ack_author != NULL ? json_object_new_string(
                                               notificationdata->ack_author) : NULL));
                json_object_object_add(notification_data_object, "ack_data",
                                       (notificationdata->ack_data != NULL ? json_object_new_string(
                                               notificationdata->ack_data) : NULL));
                json_object_object_add(notification_data_object, "host_name",
                                       (notificationdata->host_name != NULL ? json_object_new_string(
                                               notificationdata->host_name) : NULL));

                json_object_object_add(notification_data_object, "notification_type",
                                       json_object_new_int64(notificationdata->notification_type));
                json_object_object_add(notification_data_object, "start_time",
                                       json_object_new_int64(notificationdata->start_time.tv_sec));
                json_object_object_add(notification_data_object, "end_time",
                                       json_object_new_int64(notificationdata->end_time.tv_sec));
                json_object_object_add(notification_data_object, "reason_type",
                                       json_object_new_int64(notificationdata->reason_type));
                json_object_object_add(notification_data_object, "state",
                                       json_object_new_int64(notificationdata->state));
                json_object_object_add(notification_data_object, "escalated",
                                       json_object_new_int64(notificationdata->escalated));
                json_object_object_add(notification_data_object, "contacts_notified",
                                       json_object_new_int64(notificationdata->contacts_notified));
        ...
        case NEBCALLBACK_PROGRAM_STATUS_DATA:
            ...
            if ((procstats = (nebstruct_program_status_data *) data)) {
                ...
                json_object_object_add(programmstatus_object, "global_host_event_handler",
                                       (procstats->global_host_event_handler != NULL ? json_object_new_string(
                                               procstats->global_host_event_handler) : NULL));
                json_object_object_add(programmstatus_object, "global_service_event_handler",
                                       (procstats->global_host_event_handler != NULL ? json_object_new_string(
                                               procstats->global_service_event_handler) : NULL));

                json_object_object_add(programmstatus_object, "program_start",
                                       json_object_new_int64(procstats->program_start));
                json_object_object_add(programmstatus_object, "pid", json_object_new_int64(procstats->pid));
                json_object_object_add(programmstatus_object, "daemon_mode",
                                       json_object_new_int64(procstats->daemon_mode));
                ...
                json_object_object_add(programmstatus_object, "last_log_rotation",
                                       json_object_new_int64(procstats->last_log_rotation));
                json_object_object_add(programmstatus_object, "notifications_enabled",
                                       json_object_new_int64(procstats->notifications_enabled));
                json_object_object_add(programmstatus_object, "active_service_checks_enabled",
                                       json_object_new_int64(procstats->active_service_checks_enabled));
                json_object_object_add(programmstatus_object, "passive_service_checks_enabled",
                                       json_object_new_int64(procstats->passive_service_checks_enabled));
                json_object_object_add(programmstatus_object, "active_host_checks_enabled",
                                       json_object_new_int64(procstats->active_host_checks_enabled));
                json_object_object_add(programmstatus_object, "passive_host_checks_enabled",
                                       json_object_new_int64(procstats->passive_host_checks_enabled));
                json_object_object_add(programmstatus_object, "event_handlers_enabled",
                                       json_object_new_int64(procstats->event_handlers_enabled));
                json_object_object_add(programmstatus_object, "flap_detection_enabled",
                                       json_object_new_int64(procstats->flap_detection_enabled));
                //Removed in nagios 4
                json_object_object_add(programmstatus_object, "failure_prediction_enabled", json_object_new_int64(0));
                json_object_object_add(programmstatus_object, "process_performance_data",
                                       json_object_new_int64(procstats->process_performance_data));
                json_object_object_add(programmstatus_object, "obsess_over_hosts",
                                       json_object_new_int64(procstats->obsess_over_hosts));
                json_object_object_add(programmstatus_object, "obsess_over_services",
                                       json_object_new_int64(procstats->obsess_over_services));
                json_object_object_add(programmstatus_object, "modified_host_attributes",
                                       json_object_new_int64(procstats->modified_host_attributes));
                json_object_object_add(programmstatus_object, "modified_service_attributes",
                                       json_object_new_int64(procstats->modified_service_attributes));
        ...
        case NEBCALLBACK_CONTACT_STATUS_DATA:
            ...
            if ((contactstatus = (nebstruct_contact_status_data *) data)) {
                ...
                json_object_object_add(contactstatus_object, "contact_name",
                                       (tmp_contact->name != NULL ? json_object_new_string(tmp_contact->name) : NULL));

                json_object_object_add(contactstatus_object, "host_notifications_enabled",
                                       json_object_new_int64(tmp_contact->host_notifications_enabled));
                json_object_object_add(contactstatus_object, "service_notifications_enabled",
                                       json_object_new_int64(tmp_contact->service_notifications_enabled));
                json_object_object_add(contactstatus_object, "last_host_notification",
                                       json_object_new_int64(tmp_contact->last_host_notification));
                json_object_object_add(contactstatus_object, "last_service_notification",
                                       json_object_new_int64(tmp_contact->last_service_notification));
                json_object_object_add(contactstatus_object, "modified_attributes",
                                       json_object_new_int64(tmp_contact->modified_attributes));
                json_object_object_add(contactstatus_object, "modified_host_attributes",
                                       json_object_new_int64(tmp_contact->modified_host_attributes));
                json_object_object_add(contactstatus_object, "modified_service_attributes",
                                       json_object_new_int64(tmp_contact->modified_service_attributes));
        ...
        case NEBCALLBACK_CONTACT_NOTIFICATION_DATA:
            ...
            if ((cnd = (nebstruct_contact_notification_data *) data)) {
                ...
                json_object_object_add(cnd_object, "host_name",
                                       (cnd->host_name != NULL ? json_object_new_string(cnd->host_name) : NULL));
                json_object_object_add(cnd_object, "service_description",
                                       (cnd->service_description != NULL ? json_object_new_string(
                                               cnd->service_description) : NULL));
                json_object_object_add(cnd_object, "output",
                                       (cnd->output != NULL ? json_object_new_string(cnd->output) : NULL));
                //May be this will exists one day?
                json_object_object_add(cnd_object, "long_output",
                                       (cnd->output != NULL ? json_object_new_string(cnd->output) : NULL));
                json_object_object_add(cnd_object, "ack_author",
                                       (cnd->ack_author != NULL ? json_object_new_string(cnd->ack_author) : NULL));
                json_object_object_add(cnd_object, "ack_data",
                                       (cnd->ack_data != NULL ? json_object_new_string(cnd->ack_data) : NULL));
                json_object_object_add(cnd_object, "contact_name",
                                       (cnd->contact_name != NULL ? json_object_new_string(cnd->contact_name) : NULL));
        ...
        case NEBCALLBACK_CONTACT_NOTIFICATION_METHOD_DATA:
            ...
            if ((cnm = (nebstruct_contact_notification_method_data *) data)) {
                ...
                json_object_object_add(cnm_object, "host_name",
                                       (cnm->host_name != NULL ? json_object_new_string(cnm->host_name) : NULL));
                json_object_object_add(cnm_object, "service_description",
                                       (cnm->service_description != NULL ? json_object_new_string(
                                               cnm->service_description) : NULL));
                json_object_object_add(cnm_object, "output",
                                       (cnm->output != NULL ? json_object_new_string(cnm->output) : NULL));
                json_object_object_add(cnm_object, "ack_author",
                                       (cnm->ack_author != NULL ? json_object_new_string(cnm->ack_author) : NULL));
                json_object_object_add(cnm_object, "ack_data",
                                       (cnm->ack_data != NULL ? json_object_new_string(cnm->ack_data) : NULL));
                json_object_object_add(cnm_object, "contact_name",
                                       (cnm->contact_name != NULL ? json_object_new_string(cnm->contact_name) : NULL));
                json_object_object_add(cnm_object, "command_name",
                                       (cnm->command_name != NULL ? json_object_new_string(cnm->command_name) : NULL));
                json_object_object_add(cnm_object, "command_args",
                                       (cnm->command_args != NULL ? json_object_new_string(cnm->command_args) : NULL));
        ...
        case NEBCALLBACK_EVENT_HANDLER_DATA:
            ...
            if ((event_handler_data = (nebstruct_event_handler_data *) data)) {
                ...
                json_object_object_add(my_object, "timestamp",
                                       json_object_new_int(event_handler_data->timestamp.tv_sec));

                json_object *ehd_object = json_object_new_object();
                json_object_object_add(ehd_object, "host_name",
                                       (event_handler_data->host_name != NULL ? json_object_new_string(
                                               event_handler_data->host_name) : NULL));
                json_object_object_add(ehd_object, "service_description",
                                       (event_handler_data->service_description != NULL ? json_object_new_string(
                                               event_handler_data->service_description) : NULL));
                json_object_object_add(ehd_object, "output",
                                       (event_handler_data->output != NULL ? json_object_new_string(
                                               event_handler_data->output) : NULL));
                //ther is no longoutput at the moment!
                json_object_object_add(ehd_object, "long_output",
                                       (event_handler_data->output != NULL ? json_object_new_string(
                                               event_handler_data->output) : NULL));
                json_object_object_add(ehd_object, "command_name",
                                       (event_handler_data->command_name != NULL ? json_object_new_string(
                                               event_handler_data->command_name) : NULL));
                json_object_object_add(ehd_object, "command_args",
                                       (event_handler_data->command_args != NULL ? json_object_new_string(
                                               event_handler_data->command_args) : NULL));
                json_object_object_add(ehd_object, "command_line",
                                       (event_handler_data->command_line != NULL ? json_object_new_string(
                                               event_handler_data->command_line) : NULL));
                ...
                json_object_object_add(ehd_object, "early_timeout",
                                       json_object_new_int64(event_handler_data->early_timeout));
                json_object_object_add(ehd_object, "return_code",
                                       json_object_new_int64(event_handler_data->return_code));
                json_object_object_add(ehd_object, "execution_time",
                                       json_object_new_double(event_handler_data->execution_time));

                json_object_object_add(ehd_object, "start_time",
                                       json_object_new_int64(event_handler_data->start_time.tv_sec));
                json_object_object_add(ehd_object, "end_time",
                                       json_object_new_int64(event_handler_data->end_time.tv_sec));
...
void dump_object_data() {
    ...
    for (temp_timeperiod = timeperiod_list; temp_timeperiod != NULL; temp_timeperiod = temp_timeperiod->next) {
        ...
        for (x = 0; x < 7; x++) {
            ...
            for (temp_timerange = temp_timeperiod->days[x];
    ...
    for (temp_contact = contact_list; temp_contact != NULL; temp_contact = temp_contact->next) {
        ...
        json_object_object_add(my_object, "alias",
                               (temp_contact->alias != NULL ? json_object_new_string(temp_contact->alias) : NULL));
        json_object_object_add(my_object, "email",
                               (temp_contact->email != NULL ? json_object_new_string(temp_contact->email) : NULL));
        json_object_object_add(my_object, "pager",
                               (temp_contact->pager != NULL ? json_object_new_string(temp_contact->pager) : NULL));
        json_object_object_add(my_object, "host_notification_period",
                               json_object_new_string(temp_contact->host_notification_period));
        json_object_object_add(my_object, "service_notification_period",
                               json_object_new_string(temp_contact->service_notification_period));
        json_object_object_add(my_object, "notify_on_service_downtime", json_object_new_int(
                flag_isset(temp_contact->service_notification_options, OPT_DOWNTIME)));
        json_object_object_add(my_object, "notify_on_host_downtime",
                               json_object_new_int(flag_isset(temp_contact->host_notification_options, OPT_DOWNTIME)));
        json_object_object_add(my_object, "host_notifications_enabled",
                               json_object_new_int(temp_contact->host_notifications_enabled));
        json_object_object_add(my_object, "service_notifications_enabled",
                               json_object_new_int(temp_contact->service_notifications_enabled));
        json_object_object_add(my_object, "can_submit_commands",
                               json_object_new_int(temp_contact->can_submit_commands));

        json_object_object_add(my_object, "notify_on_service_unknown", json_object_new_int(
                flag_isset(temp_contact->service_notification_options, OPT_UNKNOWN)));
        json_object_object_add(my_object, "notify_on_service_warning", json_object_new_int(
                flag_isset(temp_contact->service_notification_options, OPT_WARNING)));
        json_object_object_add(my_object, "notify_on_service_critical", json_object_new_int(
                flag_isset(temp_contact->service_notification_options, OPT_CRITICAL)));
        json_object_object_add(my_object, "notify_on_service_recovery", json_object_new_int(
                flag_isset(temp_contact->service_notification_options, OPT_RECOVERY)));
        json_object_object_add(my_object, "notify_on_service_flapping", json_object_new_int(
                flag_isset(temp_contact->service_notification_options, OPT_FLAPPING)));
        json_object_object_add(my_object, "notify_on_host_unreachable", json_object_new_int(
                flag_isset(temp_contact->host_notification_options, OPT_UNREACHABLE)));
        json_object_object_add(my_object, "notify_on_host_down",
                               json_object_new_int(flag_isset(temp_contact->host_notification_options, OPT_DOWN)));
        json_object_object_add(my_object, "notify_on_host_recovery",
                               json_object_new_int(flag_isset(temp_contact->host_notification_options, OPT_RECOVERY)));
        json_object_object_add(my_object, "notify_on_host_flapping",
                               json_object_new_int(flag_isset(temp_contact->host_notification_options, OPT_FLAPPING)));
        ...
        for (x = 0; x < 6; x++) {
            json_object_array_add(address_array,
                                  (temp_contact->address[x] != NULL ? json_object_new_string(temp_contact->address[x])
                                                                    : NULL));
        }
        ...
        //commandsmember *contactcommand = NULL;
        for (contactcommand = temp_contact->host_notification_commands;
             contactcommand != NULL; contactcommand = contactcommand->next) {
            ...
            if (_command != NULL) {
                ...
                json_object_object_add(hostcommand_object, "command_line",
                                       json_object_new_string(_command->command_line));
        ...
        //commandsmember *contactcommand = NULL;
        for (contactcommand = temp_contact->service_notification_commands;
             contactcommand != NULL; contactcommand = contactcommand->next) {
            ...
            if (_command != NULL) {
                ...
                json_object_object_add(servicecommand_object, "command_line",
                                       json_object_new_string(_command->command_line));
    ...
    for (temp_contactgroup = contactgroup_list;
        ...
        for (temp_contactsmember = temp_contactgroup->members;
            ...
            json_object_array_add(contactgroup_contact_members_array,
                                  (temp_contactsmember->contact_name != NULL ? json_object_new_string(
                                          temp_contactsmember->contact_name) : NULL));
        }
    ...
    for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
        ...
        json_object_object_add(my_object, "flap_detection_on_up",
                               json_object_new_int(flag_isset(temp_host->flap_detection_options, OPT_UP)));
        json_object_object_add(my_object, "flap_detection_on_down",
                               json_object_new_int(flag_isset(temp_host->flap_detection_options, OPT_DOWN)));
        json_object_object_add(my_object, "flap_detection_on_unreachable",
                               json_object_new_int(flag_isset(temp_host->flap_detection_options, OPT_UNREACHABLE)));

        json_object_object_add(my_object, "notify_on_down",
                               json_object_new_int(flag_isset(temp_host->notification_options, OPT_DOWN)));
        json_object_object_add(my_object, "notify_on_unreachable",
                               json_object_new_int(flag_isset(temp_host->notification_options, OPT_UNREACHABLE)));
        json_object_object_add(my_object, "notify_on_recovery",
                               json_object_new_int(flag_isset(temp_host->notification_options, OPT_RECOVERY)));
        json_object_object_add(my_object, "notify_on_flapping",
                               json_object_new_int(flag_isset(temp_host->notification_options, OPT_FLAPPING)));
        json_object_object_add(my_object, "notify_on_downtime",
                               json_object_new_int(flag_isset(temp_host->notification_options, OPT_DOWNTIME)));

        json_object_object_add(my_object, "stalk_on_up",
                               json_object_new_int(flag_isset(temp_host->stalking_options, OPT_UP)));
        json_object_object_add(my_object, "stalk_on_down",
                               json_object_new_int(flag_isset(temp_host->stalking_options, OPT_DOWN)));
        json_object_object_add(my_object, "stalk_on_unreachable",
                               json_object_new_int(flag_isset(temp_host->stalking_options, OPT_UNREACHABLE)));
        ...
        for (temp_contactgroupsmember = temp_host->contact_groups;
             temp_contactgroupsmember != NULL; temp_contactgroupsmember = temp_contactgroupsmember->next) {
            json_object_array_add(contactgroups_array,
                                  (temp_contactgroupsmember->group_name != NULL ? json_object_new_string(
                                          temp_contactgroupsmember->group_name) : NULL));
        }
        ...
        for (temp_contacts = temp_host->contacts; temp_contacts != NULL; temp_contacts = temp_contacts->next) {
            json_object_array_add(contacts_array, (temp_contacts->contact_name != NULL ? json_object_new_string(
                    temp_contacts->contact_name) : NULL));
        }
        ...
        for (temp_customvar = temp_host->custom_variables;
             temp_customvar != NULL; temp_customvar = temp_customvar->next) {
            json_object_object_add(host_customvariables, temp_customvar->variable_name,
                                   json_object_new_string(temp_customvar->variable_value));
        }
    ...
    for (temp_hostgroup = hostgroup_list; temp_hostgroup != NULL; temp_hostgroup = temp_hostgroup->next) {
        ...
        json_object_object_add(my_object, "group_name",
                               (temp_hostgroup->group_name != NULL ? json_object_new_string(temp_hostgroup->group_name)
                                                                   : NULL));
        json_object_object_add(my_object, "alias",
                               (temp_hostgroup->alias != NULL ? json_object_new_string(temp_hostgroup->alias) : NULL));
    ...
    for (temp_service = service_list; temp_service != NULL; temp_service = temp_service->next) {
        ...
        json_object_object_add(my_object, "flap_detection_on_ok",
                               json_object_new_int(flag_isset(temp_service->flap_detection_options, OPT_OK)));
        json_object_object_add(my_object, "flap_detection_on_warning",
                               json_object_new_int(flag_isset(temp_service->flap_detection_options, OPT_WARNING)));
        json_object_object_add(my_object, "flap_detection_on_unknown",
                               json_object_new_int(flag_isset(temp_service->flap_detection_options, OPT_UNREACHABLE)));
        json_object_object_add(my_object, "flap_detection_on_critical",
                               json_object_new_int(flag_isset(temp_service->flap_detection_options, OPT_CRITICAL)));

        json_object_object_add(my_object, "notify_on_unknown",
                               json_object_new_int(flag_isset(temp_service->notification_options, OPT_UNKNOWN)));
        json_object_object_add(my_object, "notify_on_warning",
                               json_object_new_int(flag_isset(temp_service->notification_options, OPT_WARNING)));
        json_object_object_add(my_object, "notify_on_critical",
                               json_object_new_int(flag_isset(temp_service->notification_options, OPT_CRITICAL)));
        json_object_object_add(my_object, "notify_on_recovery",
                               json_object_new_int(flag_isset(temp_service->notification_options, OPT_RECOVERY)));
        json_object_object_add(my_object, "notify_on_flapping",
                               json_object_new_int(flag_isset(temp_service->notification_options, OPT_FLAPPING)));
        json_object_object_add(my_object, "notify_on_downtime",
                               json_object_new_int(flag_isset(temp_service->notification_options, OPT_DOWNTIME)));

        json_object_object_add(my_object, "stalk_on_ok",
                               json_object_new_int(flag_isset(temp_service->stalking_options, OPT_OK)));
        json_object_object_add(my_object, "stalk_on_warning",
                               json_object_new_int(flag_isset(temp_service->stalking_options, OPT_WARNING)));
        json_object_object_add(my_object, "stalk_on_unknown",
                               json_object_new_int(flag_isset(temp_service->stalking_options, OPT_UNKNOWN)));
        json_object_object_add(my_object, "stalk_on_critical",
                               json_object_new_int(flag_isset(temp_service->stalking_options, OPT_CRITICAL)));

        //Get parent services
        servicesmember *temp_parent_services = temp_service->parents;
        json_object *parent_services_array = json_object_new_array();
        for (temp_parent_services = temp_service->parents;
             temp_parent_services != NULL; temp_parent_services = temp_parent_services->next) {
            json_object *parent_service = json_object_new_object();
            json_object_object_add(parent_service, "host_name",
                                   json_object_new_string(temp_parent_services->host_name));
            json_object_object_add(parent_service, "service_description",
                                   json_object_new_string(temp_parent_services->service_description));
        ...
        for (temp_customvar = temp_service->custom_variables;
             temp_customvar != NULL; temp_customvar = temp_customvar->next) {
            json_object_object_add(service_customvariables, temp_customvar->variable_name,
                                   json_object_new_string(temp_customvar->variable_value));
        }
    ...
    for (temp_servicegroup = servicegroup_list;
        ...
        json_object_object_add(my_object, "group_name", (temp_servicegroup->group_name != NULL ? json_object_new_string(
                temp_servicegroup->group_name) : NULL));
        json_object_object_add(my_object, "alias",
                               (temp_servicegroup->alias != NULL ? json_object_new_string(temp_servicegroup->alias)
                                                                 : NULL));
        ...
        for (temp_servicegroupmember = temp_servicegroup->members;
             temp_servicegroupmember != NULL; temp_servicegroupmember = temp_servicegroupmember->next) {
            ...
            json_object_object_add(member, "service_description",
                                   json_object_new_string(temp_servicegroupmember->service_description));
